...
exports.extender = function() {
  ...
  K.settings.socket = K.settings.socket || (K.settings.socket = {});
  K.settings.socket._channels = K.settings.socket._channels || (K.settings.socket._channels = {});
  var _channels = K.settings.socket._channels || {};
...
//      , 'flashsocket'
    ...
    _.each(channels, function(channel, channelId){
      ...
      console.info('Socket initialize '+ channelId, channel);
      ...
      ioChannel.on('connection', function(socket) {
        ...
        _.each(channel.on.server || (channel.on.server = {}), function(callback, eventName) {
          socket.on(eventName, callback);
          K.settings.socket.channels[eventName] = {};
        });
