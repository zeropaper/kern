...
(function(wexpows){
  ...
  methods.initialize = function(options) {
    ...
    var Router = Backbone.Router.extend({
      ...
      applyfile: function(filepath){
        console.info('Navigate to '+ filepath, K.bone, K.paths, window.location.hash);
  ...
  methods.contentEntry.watchable = function() {
    if (this.watched || this.error) return false;
    var validMime = !this.has('mime') || this.Kern.settings.contentWatchExp.test(this.get('mime'));
    // console.info('Valid mime?', this.id, validMime);
    return validMime;
  };
  ...
  methods.contentEntry.cover = function(prefix, toFormat) {
    var CE = this,
    ...
    if (!isImage && !CE.isDir()) CE.image = prefix + CE.id + '.png';
    if (CE.image) return CE.image;
    
    if (isImage) CE.image = prefix + CE.id + toFormat;
    if (CE.image) return CE.image;

    /*
    if (/\.(pdf|psd|svg)$/i.test(CE.get('name'))) CE.image = prefix + CE.id +'.png';
    if (CE.image) return CE.image;
    */
    console.info('Could not find/create a cover for '+ CE.id +', trying with covers');
    var found = CE.children.find(function(model){
      return /^cover\./i.test(model.get('name')) || CE.image;
    });
    if (found) CE.image = found.cover(prefix, toFormat);
    if (CE.image) return CE.image;
    
    // for (var m in CE.children.models) {
    //   var model = CE.children.models[m];
    //   if (/^cover\./i.test(model.get('name'))) CE.image = model.cover(prefix, toFormat);
    //   if (CE.image) return CE.image;
    // }
    
    console.info('.... no luck, trying with children');
    for (var m in CE.children.models) {
  ...
  methods.renderEntryChildren = function(options) {
    ...
    options.file.children.each(function(model, m) {
      ...
      if (!model.has('hidden') || !model.get('hidden')) {
        var coverArt = model.cover()
          , mimeClass = model.get('mime') ? model.get('mime').split(/[^a-z0-9-]/ig).join('-') : 'directory'
        ;
        if (!coverArt) mimeClass += ' no-art';

        /*
        var tmpl = '<li style="<%= cover %>" class="<%= cssClass %>">'
          + '<a href="<%= url %>"><span class="inner">'
          + '<span class="title"><%= title %></span>'
          + '<span class="info"><%= children %></span>'
          + '</span></a>'
          + '</li>'
        ;
        */
        var tmpl = !onServer ? K.$('#contententry-template').html() : K.htmlEntityDecode(K.assets.templates['contententry-template']);
        
        console.info('contententry-template content', tmpl);
        ...
        out.push(compiled({
          ...
          cssClass: mimeClass, 
          children: 
                    (model.children.length  ? '<span class="count">'+ model.children.length +'</span>' : '')
        ...
        // out.push('<a href="'+ model.id +'"><span class="inner">');
  ...
  methods.renderBreadcrumb = function(options) {
    ...
    function breadcrumb() {
      ...
      console.info('renderBreadcrumb', isRoot, options.file.get('parents'), options.files);
//      if (_.isUndefined(K.paths[options.current])) return options.current;
...
//        console.info('breadcrumb entry - '+p+' '+parents[p]);
  ...
  */
  ...
  methods.findIndex = function(file) {
    K = K || this;
    if (!_.isUndefined(K.paths[file.id+'/index.html'])) file = K.paths[file.id+'/index.html'];
    if (!_.isUndefined(K.paths[file.id+'index.html'])) file = K.paths[file.id+'index.html'];
    return file;
  };
  ...
  methods.applyFile = function(fileURL) {
    ...
    var file = (!fileURL || fileURL == '' || fileURL == '/') ? K.struct : (_.isString(fileURL) ? K.paths[fileURL] : false);
    console.info('applyFile found file ', file);
    ...
    if (!file.has('error')) {
      ...
      console.info('File to be applied ', file, K.bone, window.location.hash, K.currentFile && (K.currentFile.id == file.id));
      K.currentFile = file;
    }
...
//    }));
  ...
  methods.apply = function(file){
    ...
    for (var selector in methods.rules) {
      //console.info('Applying rule '+ selector);
      try {
        methods.rules[selector].call($(selector), data, true, K);
      }
      catch (e) {
  ...
  methods.analyse = function(str, options) {
    ...
    for (var selector in methods.rules) {
      ...
      methods.rules[selector].call($(selector, $f), parsed, false, K);//, options || {});
    }
    return parsed;
  };
...
//      this.template = _.template();
  ...
  methods.rules['[src]:not(base),[href]:not(base)'] = function(analysed, place) {
  ...
  methods.rules['video'] = function(analysed, place) {
  ...
  methods.rules['[role=static-dev-scripts]'] = function(analysed, place) {
    this.remove();
  };
  
  methods.rules['[role=document-title], .page-title, title'] = function(analysed, place) {
    if (!place) {
  ...
  methods.rules['[role=document-body], body'] = function(analysed, place) {
...
//      console.info('Found '+ this.last().html());
  ...
  methods.rules['[role=document-aside]'] = function(analysed, place) {
    analysed.regions = analysed.regions || {};
    if (!place) {
  ...
  methods.rules['meta[property^="og:"]'] = function(analysed, place) {
  ...
  methods.rules['script'] = function(analysed, place) {
    if (!onServer) return;
  ...
  methods.rules['style, link[rel="stylesheet"]'] = function(analysed, place) {
    if (!onServer) return;
  ...
  methods.rules['[type="kern/lib"]'] = function(analysed, place) {
  ...
  methods.rules['#meta'] = function(analysed, place) {
    if (!place) return;
    var info = [];
  ...
  methods.rules['.kern-remove'] = function(analysed) { this.remove(); };
