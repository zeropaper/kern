(function(wexpows) {
  ...
  methods.applyFile = function(fileURL) {
    ...
    if (!file.has('error')) {
      ...
      console.info('File to be applied ', file, K.bone, window.location.hash,
          K.currentFile && (K.currentFile.id == file.id));
      K.currentFile = file;
    }
      ...
      this
          .each(function() {
            var $s = $(this), script = {
              ...
              type : $s.attr('type'),
              src : $s.attr('src'),
            }, name = (script.src ? script.src : 'inline'
                + analysed.scripts.length);
