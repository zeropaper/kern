...
//      return require('path').exists(this.absPath());
    ...
    toJSON: function() {
      return _.extend(_.clone(this.attributes), {
        children: _.reject(this.children.toJSON(), function(child){ return child.name.substr(0, 1) == '.' || child.name.substr(child.name.length - 1, child.name.length) == '~'; })
      });
    },
    title: function() {
...
//        console.info('breadcrumb entry - '+p+' '+parents[p]);
  ...
  methods.errorEntry = function(err) {
    ...
    var error = {
      ...
      children: K.paths['/'].children.toJSON(),
      regions: {
