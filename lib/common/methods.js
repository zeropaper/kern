(function(wexpows) {
  ...
  ce.initialize = function(attributes, options) {
    ...
    // console.info('CE.Kern.paths', CE.Kern.paths);
  ...
  cc.initialize = function(models, options) {
    // console.info('Content Entry Children options', typeof options.Kern, _.keys(options).join("\n"));
    // console.trace();
  ...
  function AJAXLinkApply() {
    ...
    try {
      ...
      console.error(e);
      console.error('Could not navigate to !'+ newURL);
    }
    return false;
  }
  ;
  ...
  function assetsReload(assets, t) {
    console.error('assetsReload called')
    for ( var a in assets) {
      switch (t) {
      case 'styles':
        if (assets[a].href) {
  ...
  kern.applyFile = function(fileURL) {
    ...
    if (!file.has('error')) {
      ...
      console.info('K.currentPage.id, file.id', K.currentPage ? K.currentPage.id : '/', file.id, K.currentPage ? K.currentPage.id : '/' == file.id);
  ...
  kern.apply = function(file) {
    ...
    console.group('Applying rules on '+ data.id, data);
