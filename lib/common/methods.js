...
//        , root: '!'+K.settings.pageAtStart
  ...
  methods.contentEntry.cover = function() {
    ...
    for (var m in CE.children.models) {
      ...
      if (/cover\./ig.test(model.get('name'))) return model.get('url');
    }
    return false;
  };
  ...
  methods.renderEntryChildren = function(options) {
    ...
    options.file.children.each(function(model, m) {
      if (!model.get('hidden')) {
        ...
        gallery.push('<img src="'+ model.get('url') +'" />');
      }
    });
    ...
    if (gallery.length) {
      content = content + '<div class="gallery">'+ gallery.join("\n") +'</div>';
    }
...
//      console.info((place ? 'Applyed' : 'Found') +' '+ _.keys(analysed.refs).length +' references');//, analysed.refs);
  ...
  methods.renderfileInfo = function(file) {
    ...
    _.each({
      ...
      if (name == 'parents') {
        p.push('<div class="prop '+name+'"><label>'+title+': </label> <span>'+file[name].join(', ')+'</span></div>');
      }
      else if (name == 'regions') {
        _.each(file.regions, function(content, region){
          p.push('<div class="prop '+name+'"><label>'+title+': '+region+'</label> <span>'+(String(content).length)+'</span></div>');
        });
      }
      else {
