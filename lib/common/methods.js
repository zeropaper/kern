(function(wexpows) {
  var methods = {}, ce = {} // contentEntry model description
  , cc = {} // contentChildren collection description
  , cp = {} // contentParents collection description
  , models = {}, collections = {}, views = {}, onServer = typeof window === 'undefined', AJAXLinkSelector = 'a[href^="/"]:not([href^="//"],.ajax-processed)', _, K, socket, AJAXCache = {
    elements : {},
    urls : {}
  };
  ...
  ce.initialize = function(attributes, options) {
    ...
    // if (!CE.has('url')) {
    // CE.attributes.url = (CE.attributes.parents || ['']).join('/') + '/' +
    // CE.basename();
    // }
  ...
  methods.contentEntry = ce;
  methods.contentChildren = cc;
  methods.contentParents = cp;
...
//  /**
//   * Returns true if the content entry isn't already watched, has a mime type
//   * and it satisfy the watch RegExp or is a directory (no mime)
//   */
//  ce.watchable = function() {
//    if (this.watched || this.error)
//      return false;
//    var validMime = !this.has('mime')
...
//  };
  ...
  ce.cover = function(prefix, toFormat) {
     ...
     */
    console.info('Could not find/create a cover for ' + CE.id
        + ', trying with covers');
  ...
  /**
   * Function methods.applyFile Apply the information about a file into the page
   */
  ...
  /**
   * Function: methods.apply Parameters: file - a hash object containing
  ...
  methods.analyse = function(str, options) {
    ...
    var $f = $('<div>' + str + '</div>');
    for ( var selector in methods.rules) {
      K.log('debug', 'content', 'Looking for selector: '+ selector, $(selector, $f).length);
      methods.rules[selector].call($(selector, $f), parsed, false, K);
    }
    return parsed;
  };
  ...
  methods.rules['[role="site-name"]'] = function(analysed, place, K) {
    if (place && this.length) {
      this.text(K.settings.appName);
    }
  };
  ...
  methods.rules['[role=document-title], .page-title, title'] = function(
      analysed, place) {
    if (!place) {
  ...
  methods.rules['.kern-template'] = function(analysed, place) {
    if (!onServer)
      return;
    ...
    if (!place) {
      this
          .each(function() {
            var $s = $(this);
            var name = ($s.attr('id') ? $s.attr('id') : 'inline'+ _.size(analysed.templates));
            analysed.templates[name] = $s.html();
          });
    } else {
    }
  };

  methods.rules['.kern-script'] = function(analysed, place) {
    if (!onServer)
      return;
    ...
    if (!place) {
      this
          .each(function() {
            console.info('----------------  kern-script found');
            var $s = $(this), script = {
              content : $s.html(),
              type : $s.attr('type'),
              src : $s.attr('src'),
            }, name = (script.src ? script.src :
                ($s.attr('id') ? $s.attr('id') : 'inline'+ _.size(analysed.scripts)));
  ...
  methods.rules['.kern-style'] = function(analysed, place) {
    if (!onServer)
      return;
    ...
    if (!place) {
      this
          .each(function() {
            console.info('----------------  kern-style found');
            var $s = $(this), style = {
              content : $s.html(),
              type : $s.attr('type'),
              href : $s.attr('href'),
            }, name = (style.href ? style.href :
                ($s.attr('id') ? $s.attr('id') : 'inline'+ _.size(analysed.styles)));
  ...
  methods.rules['.kern-remove'] = function(analysed) {
    this.remove();
  };
  ...
  methods.fs.hasChildren = function(f, invisible) {
    if (!methods.fs.isDir(f))
      return false;
    for ( var c in f.children) {
      if (!methods.fs.isEmpty(f.children[c]))
        return true;
    }
    return true;
  };
  ...
  methods.fs.hasIndex = function(f) {
    if (methods.fs.isFile(f) || !methods.fs.isDir(f))
      return false;
...
})(typeof exports === 'undefined' ? this.KernMethods = this.KernMethods || {}
    : exports);
