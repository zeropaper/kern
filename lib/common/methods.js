...
(function(wexpows){
  ...
  function inTrail(deepest, checked) {
    if (deepest.length < checked.length) return false;
    deepest = K.routes[deepest];
    checked = K.routes[checked];
    if (!deepest || !checked) return false;
    for (var i in checked) {
      if (deepest[i] != checked[i]) {
        return false;
      }
    }
    return true;
...
//      , K = this
  ...
  function fileListItem(file, routes, childrenCallback) {
    ...
    out.push('<li'+(inTrail(file.url, ) ? ' class="trail"' : '')+'>');
