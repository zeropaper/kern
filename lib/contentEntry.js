...
function queueProcess() {
  ...
  if (_.isFunction(params.parser)) {
    K.log('debug', 'content', 'Processing queued job', queue.length, current);
    params.parser.call(params.context, params.info, params.cb, false);
  }
  else {
    K.log('error', 'content', 'No parser defined for ', current);
    current = false;
  }
  if (!queue.length) return clearInterval(runningQueue);
}

function queueAdd(context, info, parser, cb) {
  queue.push({context: context, info: info, parser: parser, cb: cb});
  var K = context.Kern;
  
  K.log('debug', 'content', "Adding to queue "+ context.get('id'), queue.length, runningQueue);
...
//    if (queue.length >= 8) {
      K.log('info', 'content', 'Postponed scan');
      queueAdd(CE, info, parser, function(err, collected){
        current = false;
        if (err) return;
        CE.set(collected, {silent: false});
      });
