  ...
  if (this.isDir()) {
    ...
    if (_.isFunction(parser)) {
      parser.call(CE, info, function(err, collected){
        ...
        // console.log('info', 'content', 'Collected information for '+ CE.id, _.toArray(collected.regions).length, 'hidden? '+ collected.hidden, 'noMenu? '+ collected.noMenu, CE.collection.length);
        K.log('debug', 'content', 'Collected', collected);

        if (path.basename(absPath) == 'roadmap.html') {
          K.log('info', 'roadmap', 'Collected', collected);
        }
