...
ce.toJSON = function() {
  ...
  _.each(attrs, function(attr, a){
    attrs[a] = (attr && _.isFunction(attr.toJSON) ? attr.toJSON() : attr);
  });
...
ce.scan = function(cb, quiet) {
  ...
  if (this.isDir()) {
    ...
    if (CE.get('depth') < CE.Kern.settings._crawlMaxDepth) {
...
ce.initialize = function(attributes, options) {
  ...
  if (options.isRoot) {
    ...
    CE.bind('initialized', function() {
      // EVENT: Kern, initialized, triggered when Kern has been initialized, generally after the complete initialization of the root ContentEntry
  ...
  // EVENT: ContentEntry, initialized, triggered when the content entry has been initialized
