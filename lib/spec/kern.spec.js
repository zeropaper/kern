...
describe("Kern", function() {
  ...
  describe("configuration", function(){
    ...
    it('Has a content directory ('+ settings.contentDir +')', function() {
      expect(kern.contentDir()).toBe(settings.contentDir);
    });
    it('The content directory ('+ settings.contentDir +') exists', function() {
      expect(kern.contentDir()).pathToExists();
      expect(kern.contentDir()).toBeADirectory();
    });

    it('Has a public directory ('+ settings.publicDir +')', function() {
      expect(kern.publicDir()).toBe(settings.publicDir);
    });
    it('The public directory ('+ settings.publicDir +') exists', function() {
      expect(kern.publicDir()).pathToExists();
      expect(kern.publicDir()).toBeADirectory();
    });

    it('Has a cache directory ('+ settings.cacheDir +')', function() {
      expect(kern.cacheDir()).toBe(settings.cacheDir);
    });
    it('The cache directory ('+ settings.cacheDir +') exists', function() {
  ...
  describe("Content models", function(){
    ...
    waitsFor(function(){
      if (demoEntry.scanned) {
        // console.info('It is scanned.... ', demoEntry.toJSON());
        return true;
      }
      return false;
    ...
    it ("All common scripts are runned successfully.", function() {
      console.info('CE scripts', demoEntry.get('scripts'));
      var commonScripts = _.filter(demoEntry.get('scripts'), function(i){
        console.info('Common script?', i.id, i.runs);
        return i.runs;
      });
      console.info('commonScripts', _.pluck(commonScripts, 'id').join(', '));
      // _.each(commonScripts, function(script, id) {
      //   expect(script.error).toBeUndefined();
      // });
      var failed = _.filter(commonScripts, function(i){
        console.info('Common script error?', i.id, i.error);
        if (i.error) {
          throw i.error;
        }
    ...
    waitsFor(function(){
      console.info('Waiting....', !_.isUndefined(demoEntry.attributes.data), !_.isUndefined(demoEntry.attributes.error));
    ...
    it ("The demo entry to have a 'data' attribute.", function() {
      console.info('Data ?', demoEntry.attributes.data);
  ...
  describe("Serving", function(){
    ...
    // waitsFor(function(){
    //   if (code) {
    //     console.info('We got a response!!!!', code);
    //   }
    //   return code;
    ...
    //     console.info('We got a response!!!!', response.statusCode, _res.statusCode);
    //   }
