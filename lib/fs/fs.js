...
var childProcessTask = Backbone.Model.extend({
  ...
  initialize: function(attributes, options) {
    console.info('Initializing task', _.keys(attributes));
    var self = this;
...
function mediaParser(info, cb) {
  ...
  /*
  childProcessesQueue.add([{
    subject: CE,
    callback: require('child_process').exec,
    arguments: [cmd, function(error, stdout, stderr) {
      console.info('_.isEqual(CE, this), CE == this', _.isEqual(CE, this), CE == this);
      if (error) {
        console.error('mediaParser error');
        return;//throw error;
      }
      ...
      if (/^image\//ig.test(info.mime)) {
        info.regions.main = '<img src="'+ info.url +'" class="original" />';
      }
      else if (/^video\//ig.test(info.mime)) {
        info.regions.main = '<video><source src="'+ info.url +'" type="'+info.mime+'" /></video>';
      }
      else if (/^audio\//ig.test(info.mime)) {
        info.regions.main = '<audio><source src="'+ info.url +'" type="'+info.mime+'" /></audio>';
      }
      
      console.info("Found meta ", _.keys(info.meta || {}));
