...
exports.errorTemplate = function(code) {
  ...
  try {
    ...
    K.log('error', 'Can not read ' + __publicdir + '/error-' + (code || 500) + '.html');
    try {
      output = fs.readFileSync(__publicdir + '/error.html', 'utf-8');
    } 
    catch (e) {
      K.log('error', 'Can not read ' + __publicdir + '/error.html');
      try {
        output = fs.readFileSync(__publicdir + '/index.html', 'utf-8');
      } 
      catch (e) {
        K.log('error', 'Can not read ' + __publicdir + '/index.html');
      }
    }
  }
...
exports.errorTemplateReplacements = function(string, replacements) {
  ...
  for (var s in replacements) {
    ...
    //K.log('log', 'Search "<!-- '+s+' -->" and replace with "'+r+'"');
...
//  throw new Error('"compile" method is deprecate');
  ...
  return function(locals) {
    ...
    if (locals.req.url) {
      ...
      //K.log('info', 'locals.currentPage is defined', 'children: '+ _.keys(file.children || {}).join(', '), 'regions: '+ _.keys(file.region ||{}).join(', '));
      // K.log('log', '"'+file.name+'"', file.regions);
      if (typeof file == 'object' && typeof file.regions == 'object') {
        // K.log('info', 'Using file attribute main as content');
        content = file.regions.main;
      }
    }
    else if (typeof options.body == 'string') {
      K.log('error', 'Using options.body as content, this should not happend');
      content = options.body;
    }
    else {
      K.log('info', 'Rendering an index page');
...
exports.fixHTML = function(html, options) {
  ...
  K.log('info', 'Fixing HTML before sending response');
  ...
  //  K.log('info', 'There is now static '+(appStates.length)+' states');
