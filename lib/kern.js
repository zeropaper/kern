(function(wexpows){
  ...
  function log() {
    ...
    if (_.isFunction(console[_.first(args)])) method = args.shift();
    
    args = args.unshift("- Kern - "+ method);
//    if (!onServer) 
//    console[method].apply({}, args);
    console.log(args);
  }
  ...
  var Kern = function(options) {
    ...
    K.log('info', 'Just a test', {"with":"an object"});
    K.log('Just a test', {"with":"an object"});
    ...
    var ThingDef = {
      ...
      validate: function(attributes) {
        if (!attributes.name) return 'A thing needs a name';
      }
    };
    ...
    K.bind('extensionloaded', function(extension){
      console.info("Extension loaded: "+ (extension.extensionName || 'anonymous') +' with props', _.keys(extension).join(", "), extension);
    });
    ...
    _.each(options.extensions || [], function(extension){
      ...
      //console.info('Extending with '+ ext.extensionName, requirementsCheck, typeof ext.extender, ext);
...
//        console.info('Triggering '+ ext.extensionName +':loaded');
        K.extensions[ext.extensionName] = ext;
        K.trigger(ext.extensionName +':loaded', ext);
      }
      else {
        console.info("Loaded an anonymous extension...");
      }
    });
