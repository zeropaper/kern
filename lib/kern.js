...
//      title: K.settings.appName
    ...
    K.bind('extensionloaded', function(extension){
      K.log("Extension loaded: "+ (extension.extensionName || 'anonymous') +' with props', _.keys(extension).join(", "), extension);
    });
    ...
    _.each(options.extensions || [], function(extension){
      ...
      //K.log('Extending with '+ ext.extensionName, requirementsCheck, typeof ext.extender, ext);
...
//        K.log('Triggering '+ ext.extensionName +':loaded');
        K.extensions[ext.extensionName] = ext;
        K.trigger(ext.extensionName +':loaded', ext);
      }
      else {
        K.log("Loaded an anonymous extension...");
      }
    });
