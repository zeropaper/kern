...
methods.registerPaths = function() {
  ...
  for (var f in K.paths) {
    console.info('Check content entry path '+ f, f.split(K.contentDir()).length >= 2, typeof f === 'string' && f != '/undefined' && f != 'undefined');
    if (f.split(K.contentDir()).length >= 2) continue;
    if (typeof f != 'string' || f == '/undefined' || f == 'undefined') continue;
    ...
    if (!_.find(K.app.routes.routes.get, function(p){
      return escape(f) == p.path;
    })) {
      K.app.get(escape(f), methods.pathRequestCallback);
    }
  }
  
  console.info('The following GET routes are registered: ', _.pluck(K.app.routes.routes.get, 'path').join("\n"));
//  console.info('The following GET routes are registered: ', K.app.routes.routes.get);
};
...
//    content = require('./fs/js').compressJS(content);
  ...
  K.app.get('*', function(req, res, next){
    //return next();
    throw new K.errors.NotFound(req.url +' could not be found'+ (" in:<div class='express'>"+ _.pluck(K.app.routes.routes.get, 'path').join("<br />") + "</div><div class='kern'>and<br/>"+ _.keys(K.paths).join("<br/>")+"</div>"));
  });
