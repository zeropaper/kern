...
kern.loadLayout = function(cb, layoutpath) {
  ...
  // console.info('******* Loaded layout from '+ layoutpath);
  ...
  function clientEscape(obj, key) {
    ...
    _.each(
      ...
      function(subObj, subKey) {
        var wipe = _.isString(subKey) && subKey.substr(0, 1) == '_';
        console.info('Procesing setting '+ subKey +'', wipe);
        if (wipe) {
          delete copy[subKey];
        }
...
kern.serve = function() {
  ...
  app.get('/kern/files.js', function(req, res, next) {
    function remove(obj) {
      ...
      // }
      // console.info(obj.id+"\t\t", _.keys(obj).join(', '), "\n", obj.scripts, "\n", JSON.stringify(obj.scripts));
    ...
    function verifJSON(o, k) {
      var str;
      console.info('verifJSON', od, kk);
      if (od >= 20) return false; 
      try {
        str = JSON.stringify(o);
        // console.info('-- str '+ od, str);
        return true;
      }
      catch (error) {
        console.error('verifJSON error', error);
        if (k) {
          kk = (kk || 'object')+'.'+k;
        }
        else {
    ...
    console.info('Sending from '+ from);
    ...
    if (verifJSON(obj)) {
      // console.info('Sending files', obj);
      res.send(obj);
    }
    else {
      res.send({
