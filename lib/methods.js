...
methods.initialize = function() {
  // K.log('Kern is initializing on the server side');
  ...
  */
  K.app.error(function(err, req, res, next) {
    ...
    return K.loadLayout(function() {
      ...
      K.log('error', '', 'Error page error', err, err.stack);
...
//      throw new K.errors.InternalError("The URL "+ url +" should not be send statically");
      K.log('info', "----- Delivering a content entry");
  ...
  // K.log('Kern.lookup(req.params)', K.lookup(req.params));
...
methods.loadTemplates = function(path) {
  ...
  ;
  K.log('Load templates for '+ path, $tmpls.length, templates.length);
  function apply() {
...
methods.applyFile = function(fileURL, req, next) {
  ...
  K.loadLayout(function() { K.log('Loaded layout', K.$('body').length > 0); });
  K.apply(file);
...
methods.registerAssets = function() {
  K = K || this;
  K.log('------- Registering assets --------');
  
  K.app.get('/kern.templates.js', function(req, res, next) {
    K.log('Templates ', req.query);
  ...
  for (var t in _.clone(K.assets)) {
    ...
    for (var a in type) {
      (function(info){
        ...
        K.log(" - - Registered "+ asset.path);
        K.app.get(asset.path, function(req, res, next) {
          //K.log('Asset requested '+ req.url, asset);
