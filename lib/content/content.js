...
Content.extender = function() {
  ...
  _.extend(ParsingRules.prototype, {
    ...
    analyse: function() {
      _.each(this.rules, function(rule) {
        rule.do($html, analysed, false);
      });
    },
    place: function() {
      _.each(this.rules, function(rule) {
        rule.do($html, analysed, true);
      });
    },
    ...
    if (!place) {
      ...
      this
        .each(function() {
          ...
          // console.info('Kern class?', $s.attr('class'), $s.hasClass('kern-common') , ($s.hasClass('kern-client') && !onServer) , ($s.hasClass('kern-server') && onServer));
          if (script.runs) {
  ...
  rulesObj['[data-kern-role=data-collection]'] = function(analysed, place) {
    if (!place) { // read the stuff
      ...
      console.info('Analysed:', analysed, K.views);//, _.keys(analysed).join(', '));
      this.each(function(){
        var $col = $(this);
        var elId = $col.attr('id');
        if (!elId) {
