...
function transcode(req, res, next) {
    ...
    if (exactSame) {
      K.log('av', "--- Sending original file "+ file.absPath());
      // return;
...
//          var input = file.absPath().split(' ').join('\\ ').split('&').join('\\&');
          var input = K.fs.escapeshellarg(file.absPath());
...
//                 if (err) return next(err);
          ...
          try {
            ...
            K.log('av', "Requested as "+ toFormat.substr(1, toFormat.length) +" "+ (proc._isStreamable ? "stream" : "transcoding"));
...
//                  if (retcode == 1 || err) K.log('av', "Error while streaming from:\n"+ input, err);
//                  if (retcode == 1) throw new Error(err);
...
//                  return res.end();
      ...
      // K.log('av', "************************            "+(toFormat ? generated : file.absPath())+"           ***************************");
...
exports.extender = function() {
  ...
  K.bind('contentprepare', function(CE){
    if (!CEisAV(CE)) return;
    K.log('info', 'av', CE.get('url') +' is an audio/video media');
