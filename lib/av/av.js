...
function CEisAV(CE) {
  ...
  && (
      ...
      (
        CE.absPath()
        && /^\.(mov|avi|mkv|ogg|ogv|mp4|wmv|rm|3gp|flac)$/i.test(path.extname(CE.absPath()))
      )
    )
  ;
  // console.info('Is "'+ CE.id +'"" a media ?', CE.get('mime'), !CE.isDir(),
  //   "CE.attributes.mime", CE.has('mime')
  //   && /^(audio|video)\//i.test(CE.get('mime')),
  //   // "CE.mime", CE.mime
  //   // && /^(audio|video)\//i.test(CE.mime),
  //   "CE.absPath()", CE.absPath()
...
exports.extender = function() {
  ...
  */
  ...
  console.info('Using '+ mejsDir +' for mediaelement js');
  K.assets.styles['/kern/av.css'] = {
    filepath: mejsDir +'/mediaelementplayer.css',
    weight: -55
  };
  K.assets.styles['/kern.mediaelementplayer.css'] = {
    filepath: mejsDir +'/mediaelementplayer.css'
  };
  ...
  K.assets.scripts['/kern.mediaelement.js'] = {
