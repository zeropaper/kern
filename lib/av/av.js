...
function onTranscoding(file, preset, toFormat, streamable) {
  ...
  console.info((streamable ? 'Streamed transcoding' : 'Transcoding') +' of '+ file.id +' to '+ toFormat +' started.');
}
function onTranscoded(file, preset, toFormat, streamable) {
  ...
  console.info((streamable ? 'Streamed transcoding' : 'Transcoding') +' of '+ file.id +' to '+ toFormat +' ended.');
}
function onTranscodingFail(file, preset, toFormat, streamable) {
  ...
  console.info((streamable ? 'Streamed transcoding' : 'Transcoding') +' of '+ file.id +' to '+ toFormat +' failed.');
}
...
function CEisAV(CE) {
  return CE.has('mime') && /$(audio|video)\//i.test(CE.get('mime'));
}
...
function transcode(req, res, next) {
    ...
    if (exactSame) {
      console.info("--- Sending original file "+ file.absPath());
      // return;
...
//          var input = file.absPath().split(' ').join('\\ ').split('&').join('\\&');
          var input = file.absPath();
          console.info("-- FFMPEG input path:\n"+ input +"\n"+ K.fs.escapeshellarg(file.absPath()));
          var proc = new ffmpeg(input);
          if (toImage) {
            return proc.takeScreenshot({
              // outputfile: generated.split(' ').join('\\ ').split('&').join('\\&'),
              outputfile: K.fs.escapeshellarg(generated),
              offset: 15
            }, function(err) {
              // if (err) {
...
//                 if (err) return next(err);
          ...
          K.trigger('transcoding', file, preset, toFormat, streamable);
          try {
            ...
            var out = streamable
              ? proc
                .writeToStream(res, function(retcode, err) {
                  console.info('Streaming '+ original +' to format '+ toFormat);
                  //if (err) return next(err);
//                  res.writeHead(retcode);
              ...
              : proc
                .renice(20)
                // .saveToFile(generated.split(' ').join('\\ ').split('&').join('\\&'), function(){
                .saveToFile(K.fs.escapeshellarg(generated), function(retcode, error){
                  if (!error) {
                    console.info('Converted '+ original +' to format '+ toFormat, retcode);
                    res.sendfile(generated, function(err){
                      console.info('Sent converted '+ original);
                      //if(err) return next(err);
                    });
                  }
                  else {
                    console.error('Could not convert '+ original +' to format '+ toFormat, retcode, error);
                  }
                })
            ;
          }
          catch (e) {
            K.trigger('transcodingfail', file, preset, toFormat, streamable);
            next(e);
          }
      ...
      // console.info("************************            "+(toFormat ? generated : file.absPath())+"           ***************************");
      // next(new K.errors.InternalServerError(original +" could not be processed"));
    });
...
exports.extender = function() {
  ...
  K.bind('contentprepare', function(CE){
    if (!CEisAV(CE)) return;
    console.info(CE.get('url') +' is an audio/video media');
