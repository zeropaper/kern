(function(wexpows){
  ...
  KernMethods.rules['av'] = function(analysed, place) {
    ...
    if (/video\//ig.test(analysed.mime)) {
      player.push('<video id="'+playerId+'" poster="/av'+url+'.png" controls="controls" preload="none">');
      player.push('<source type="'+analysed.mime+'" src="/av'+url+'" />');
      player.push('<source type="video/mp4" src="/av'+url+'.mp4" />');
      player.push('<source type="video/webm" src="/av'+url+'.webm" />');
      player.push('<source type="video/ogg" src="/av'+url+'.ogg" />');
      player.push('</video>');
      mediaelement = true;
    }
      ...
      player.push('<img id="'+playerId+'" src="'+url+'" alt="'+url+'" />');
    }
...
//        defaultVideoHeight: 270,
      ...
      try {
        $('#'+ playerId).mediaelementplayer(playerOptions)
